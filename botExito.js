...
async function exitoPrices(Product) {
    ...
    for (let i = 0; i < Math.min(5, titles.length); i++) {
        ...
        if (title && priceText && img && elementlink) {
            products.push({
                title: title,
                priceText: priceText,
            ...
            console.error('Failed to retrieve product details:', { title, priceText, img, elementlink });
        }
    }
    ...
    for (let i = 0; i < Math.min(3, products.length); i++) {
        ...
        html += '<div class="results" data-store="Exito">';
        html += `<img src="${product.img}" alt="${product.title}"></img>`;
